...
const pinterestImageDimensions: Record<string, { width: number; height: number }> = {
  "skeletal-system": { width: 736, height: 1102 },
  "muscular-system": { width: 736, height: 1104 },
  "circulatory-system": { width: 736, height: 736 },
  "respiratory-system": { width: 736, height: 949 },
  "digestive-system": { width: 736, height: 920 },
  "nervous-system": { width: 736, height: 1104 },
  "endocrine-system": { width: 736, height: 902 },
  "urinary-system": { width: 736, height: 949 },
  "reproductive-system": { width: 736, height: 920 },
  "lymphatic-immune-system": { width: 736, height: 920 },
  "integumentary-system": { width: 736, height: 920 },
};
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!system) {
    return {
      title: 'Anatomical System Not Found | Kosha Explorer Anatomy',
      description: 'The requested anatomical system detail page could not be found on Kosha Explorer. Please explore our other anatomy guides.',
      robots: { index: false, follow: false }
    };
  }
  const pageTitle = `${system.name} Detailed Guide | Anatomy Explorer | Kosha Explorer`;
  const pageDescription = `Explore the ${system.name.toLowerCase()}: Discover its key components like ${system.details.keyComponents?.[0]?.name || system.details.keyOrgans?.[0]?.name || 'main organs'}, primary functions including ${system.details.primaryFunctions?.[0]?.name.toLowerCase() || 'its vital roles'}, common disorders, and health tips. An in-depth look at this vital human body system.`;
  const pageKeywords = [
    system.name, "Human Anatomy Details", `${system.name} Functions`, `${system.name} Organs`, 
    ...(system.details.keyComponents?.map(c => c.name) || []), 
    ...(system.details.keyOrgans?.map(o => o.name) || []),
    "Body System Explained", "Anatomy Education Online", "Holistic Health Insights", "Biological Systems"
  ].filter(Boolean).slice(0, 15);

  const systemSlug = system.slug || slugify(system.name);
  const imageUrl = system.details.imageUrl || `https://placehold.co/1200x630.png?text=Detailed+${system.name}`;
  const imageAlt = system.details.imageUrl 
    ? `Detailed illustration and guide for the ${system.name}`
    : `Kosha Explorer illustration for ${system.name}`;
  const imageDimensions = system.details.imageUrl && pinterestImageDimensions[systemSlug] 
    ? pinterestImageDimensions[systemSlug] 
  ...
  return {
    ...
    openGraph: {
      title: pageTitle,
      description: pageDescription,
      url: `${siteBaseUrl}/anatomy/${systemSlug}`,
      type: 'article',
      images: [
        {
          url: imageUrl,
      ...
      article: {
        publishedTime: "2024-01-01T00:00:00.000Z", // Generic publish date
        modifiedTime: new Date().toISOString(),
        authors: [`${siteBaseUrl}/about`], // Assuming an about page for author
        section: "Anatomy",
        tags: pageKeywords,
      },
    },
...
export default function SystemDetailPage({ params }: PageProps) {
  ...
  const pageUrl = `${siteBaseUrl}/anatomy/${system.slug}`;
  const pageTitle = `${system.name} Detailed Guide | Anatomy Explorer | Kosha Explorer`;
  const pageDescription = `Explore the ${system.name.toLowerCase()}: Discover its key components like ${system.details.keyComponents?.[0]?.name || system.details.keyOrgans?.[0]?.name || 'main organs'}, primary functions, common disorders, and health tips. An in-depth look at this vital human body system.`;
  const imageUrl = system.details.imageUrl || `https://placehold.co/1200x630.png?text=Detailed+${system.name}`;
  ...
  const articleSchema = {
    ...
    "articleSection": "Anatomy",
    "keywords": [system.name, "Human Anatomy", `${system.name} Functions`, `${system.name} Organs`].join(", ")
  };
      ...
      <div className="space-y-10">
        ...
        <Card className="shadow-2xl border-primary/20 overflow-hidden bg-card">
          ...
          <CardContent className="p-6 md:p-8 space-y-10">
            ...
            <div className="w-full relative my-6 shadow-xl rounded-lg overflow-hidden border border-border max-w-md mx-auto h-[500px] sm:h-[600px] md:h-[700px]">
              <Image
                src={system.details.imageUrl || `https://placehold.co/600x800.png`}
                alt={`Detailed illustration of the ${system.name}`}
                fill
                className="object-contain"
                data-ai-hint={system.details.imageHint || system.name.split(' ')[0].toLowerCase()}
                sizes="(max-width: 640px) 100vw, (max-width: 768px) 80vw, 512px"
                priority
              />
            </div>
