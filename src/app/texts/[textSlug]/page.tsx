...
export async function generateMetadata({ params }: { params: { textSlug: string } }): Promise<Metadata> {
  ...
  if (!text) {
    return {
      title: 'Spiritual Text Not Found | Kosha Explorer',
      description: 'The requested spiritual text or philosophical writing could not be found.',
    };
  }
  const pageTitle = `${text.title} | ${text.category} | Kosha Explorer`;
  const pageDescription = `Explore the wisdom of ${text.title}, a key text in ${text.category}. Learn about its significance, key concepts like ${text.keyConcepts.slice(0,2).join(', ')}, and connection to the Pancha Koshas.`;
  const pageKeywords = [text.title, text.category, text.origin, ...text.keyConcepts].join(', ');
  const imageUrl = `https://picsum.photos/seed/${text.slug}-og/1200/630`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Illustration representing the spiritual text ${text.title}`,
        },
      ],
    },
    twitter: {
      ...
      <Card className="shadow-2xl border-primary/20 overflow-hidden bg-card">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div>
              ...
              <CardDescription className="text-md md:text-lg text-muted-foreground mt-1">
                Category: {text.category} <br />
                Origin: <span className="italic">{text.origin}</span>
        ...
        <CardContent className="p-6 md:p-8 space-y-8">
          ...
          <div className="aspect-video w-full max-w-2xl mx-auto relative my-4 shadow-lg rounded-lg overflow-hidden border border-border">
            <Image
              src={`https://picsum.photos/seed/${text.slug}/800/450`}
